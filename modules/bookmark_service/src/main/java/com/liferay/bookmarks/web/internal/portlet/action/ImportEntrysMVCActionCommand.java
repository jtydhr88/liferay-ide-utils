...
@Component(immediate = true, property = { "javax.portlet.name=" + BookmarksPortletKeys.BOOKMARKS,
		...
		"mvc.command.name=/bookmarks/import_entries" }, service = MVCActionCommand.class)
		...
		for (Map.Entry<String, Map<String, String>> mapEntry : data.entrySet()) {
			...
			if (mapEntry.getKey().equals("Bookmarks bar")) {
				...
				for (Map.Entry<String, String> urlsEntry : urls.entrySet()) {
					_bookmarksEntryService.addEntry(groupId, folderId, urlsEntry.getKey(), urlsEntry.getValue(), "",
							serviceContext);
				}

			} else {
				BookmarksFolder folder = _bookmarksFolderService.addFolder(folderId, mapEntry.getKey(), "",
						serviceContext);
				...
				for (Map.Entry<String, String> urlsEntry : urls.entrySet()) {
					...
					try {
						_bookmarksEntryService.addEntry(groupId, tempFolderId, urlsEntry.getKey(), urlsEntry.getValue(),
								"", serviceContext);
					} catch (Exception e) {
